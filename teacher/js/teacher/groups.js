...
import { 
    collection, query, where, orderBy, getDocs, doc, getDoc, updateDoc, deleteDoc, arrayUnion, addDoc, arrayRemove 
                    ...
                    <div class="p-4">
                        ...
                        <div class="grid grid-cols-2 gap-2 text-center text-[10px] uppercase font-bold tracking-wider">
                            <div class="bg-indigo-50 dark:bg-indigo-900/30 p-2 rounded border border-indigo-100 dark:border-indigo-800">
                                ...
                                <span class="text-slate-500">মোট শিক্ষার্থী</span>
                            </div>
...
Teacher.filterStudents = function(filter) {
    ...
    if (filter === 'all') {
        ...
        filteredStudents = window.currentGroupStudents.filter(s => s.status === 'active');
...
Teacher.removeStudentFromGroup = async (studentId, groupId) => {
    const result = await Swal.fire({
        ...
        text: "এই শিক্ষার্থীকে কোর্স থেকে সরানো হবে। সে পরবর্তীতে আবার জয়েন হতে পারবে।",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
    ...
    if (result.isConfirmed) {
        try {
            ...
            if (studentSnap.exists()) {
                ...
                const updatedJoinedGroups = (studentData.joinedGroups || []).filter(g => g.groupId !== groupId);
                await updateDoc(studentRef, { joinedGroups: updatedJoinedGroups });
            }
            
            Swal.fire('সরানো হয়েছে', 'শিক্ষার্থীকে কোর্স থেকে সরানো হয়েছে।', 'success');
            Teacher.viewGroupStudents(groupId);
        } catch (error) {
            Swal.fire('ত্রুটি', 'শিক্ষার্থী সরাতে ব্যর্থ: ' + error.message, 'error');
        }
    }
};
